...
#ifndef CPU_LOONGARCH64_JIT_UNI_DW_CONV_KERNEL_UTILS_HPP
#define CPU_LOONGARCH64_JIT_UNI_DW_CONV_KERNEL_UTILS_HPP
...
//#include "cpu/loongarch64/jit_avx512_core_bf16_dw_conv_kernel.hpp"
...
private:
    ...
    //        = 0;//isa == avx512_core && kernel_dt == data_type::bf16;
    ...
    using jit_kernel_t = typename utils::conditional<false,
            jit_uni_dw_conv_fwd_kernel_f32<isa>,
            jit_uni_dw_conv_fwd_kernel_f32<isa>>::type;*/
    using jit_kernel_t = jit_uni_dw_conv_fwd_kernel_f32<isa>;
    jit_kernel_t *ker_; 
};
...
status_t jit_uni_dw_conv_fwd_kernel<isa, kernel_dt>::init_conf(
    ...
    const auto def_tag
            ...
            ? nxc_tag
            : blocked_tag;*/
    const auto blocked_tag = nChw8c;
    const auto wei_tag = Goihw8g;
    const auto nxc_tag = nhwc;
    const auto def_tag = blocked_tag;
   ...
   // const bool is_bf16 = src_d.data_type() == data_type::bf16;
    ...
    jcp.nb_ch_blocking = isa == lasx ? 3 : 2;    
...
template struct jit_uni_dw_conv_fwd_kernel<lasx, data_type::f32>;
//template struct jit_uni_dw_conv_fwd_kernel<lsx, data_type::f32>;
...
private:
   /* using jit_kernel_t = typename utils::conditional<isa == avx512_core
    ...
    using jit_kernel_t = jit_uni_dw_conv_bwd_data_kernel_f32<isa>;
    jit_kernel_t *ker_;
...
status_t jit_uni_dw_conv_bwd_data_kernel<isa, kernel_dt>::init_conf(
    ...
    const auto wei_tag
            = one_of(isa, avx512_common, avx512_core) ? Goihw16g : Goihw8g;*/
    const auto dat_tag_blocked = nChw8c;
    const auto wei_tag = Goihw8g;
...
template struct jit_uni_dw_conv_bwd_data_kernel<lasx, data_type::f32>;
//template struct jit_uni_dw_conv_bwd_data_kernel<lsx, data_type::f32>;
...
private:
    ...
    using jit_kernel_t = jit_uni_dw_conv_bwd_weights_kernel_f32<isa>;
    jit_kernel_t *ker_;
};
...
status_t jit_uni_dw_conv_bwd_weights_kernel<isa, kernel_dt>::init_conf(
    ...
    const auto wei_tag
            = one_of(isa, avx512_common, avx512_core) ? Goihw16g : Goihw8g;*/
    const auto dat_tag_blocked = nChw8c; // dnnl_aBcd16b
    const auto wei_tag = Goihw8g;
    ...
    jcp.is_fast_depthwise = is_data_layout_nxc && one_of(isa, lasx);
...
//template struct jit_uni_dw_conv_bwd_weights_kernel<avx512_core, data_type::bf16>;
//template struct jit_uni_dw_conv_bwd_weights_kernel<avx512_common, data_type::f32>;
...
template struct jit_uni_dw_conv_bwd_weights_kernel<lasx, data_type::f32>;
//template struct jit_uni_dw_conv_bwd_weights_kernel<lsx, data_type::f32>;
...
#endif /* CPU_LOONGARCH64_JIT_UNI_DW_CONV_KERNEL_UTILS_HPP */
