...
*/
    ...
    void bwd_process_relu_lasx(Vmm vdiff_dst, int offt, Vmm vstore_mask) {
        ...
        add_imm(X_TMP_0, reg_soff, offt / (1 << bit_shift()), X_TMP_1);
        add_d(X_TMP_0, X_TMP_0, reg_ws);
        xvldrepl_b(vstore_mask, X_TMP_0, 0);
        //vpand(vstore_mask, vstore_mask, ptr[rip + l_relu_mask_avx2]);
        pcaddi(X_TMP_0, l_relu_mask_avx2);
        xvld(v_tmp0, X_TMP_0, 0);
        xvand_v(vstore_mask, vstore_mask, v_tmp0);
        ...
        if(ldFlag == FLAG_XVLD) {
            switch(tail_) {
                ...
                case 3: xvldrepl_d(xvreg, X_TMP_0, 0); 
                        ld_w(X_TMP_1, X_TMP_0, 4*2);
                        xvinsgr2vr_w(xvreg, X_TMP_1, 2);
                        break;
                case 4: xvldrepl_d(xvreg, X_TMP_0, 0); 
                        ld_d(X_TMP_1, X_TMP_0, 4*2);
                        xvinsgr2vr_d(xvreg, X_TMP_1, 1);
                        break;
                case 5: xvldrepl_d(xvreg, X_TMP_0, 0); 
                        ld_d(X_TMP_1, X_TMP_0, 4*2);
                        xvinsgr2vr_d(xvreg, X_TMP_1, 1);
                        ld_w(X_TMP_1, X_TMP_0, 4*4);
                        xvinsgr2vr_w(xvreg, X_TMP_1, 4);
                        break;
                case 6: xvldrepl_d(xvreg, X_TMP_0, 0); 
                        ld_d(X_TMP_1, X_TMP_0, 4*2);
                        xvinsgr2vr_d(xvreg, X_TMP_1, 1);
                        ld_d(X_TMP_1, X_TMP_0, 4*4);
                        xvinsgr2vr_d(xvreg, X_TMP_1, 2);
                        break;
                case 7: xvldrepl_d(xvreg, X_TMP_0, 0); 
                        ld_d(X_TMP_1, X_TMP_0, 4*2);
                        xvinsgr2vr_d(xvreg, X_TMP_1, 1);
                        ld_d(X_TMP_1, X_TMP_0, 4*4);
                        xvinsgr2vr_d(xvreg, X_TMP_1, 2);
                        ld_w(X_TMP_1, X_TMP_0, 4*6);
...
*/
    ...
    void uni_xvldst_maybe_tail(const XVReg &xvreg, const XReg &r1, bool ldFlag=FLAG_XVLD, const XReg &r2=zero int32_t off=0) {
...
*/
        ...
        auto variance_compute = [=](int num_ch_blks, int num_spat_pts) {
            ...
            for (int spat_pt = 0; spat_pt < num_spat_pts; ++spat_pt) {
                ...
                for (int ch_idx = 0; ch_idx < num_ch_blks; ++ch_idx) {
                    ...
                    uni_xvldst_maybe_tail(vmean, mean_ptr(coff), FLAG_XVLD);
            ...
            L(spatial);
            {
                ...
                for (int idx = 0; idx < num_ch_blks; ++idx) {
                    ...
                    if (with_relu_inf_only) { // --attr=post_ops='relu'
                        ...
                        //fwd_process_relu_avx512_common(Vmm(idx));  //TODO: avx512 ?
                    }

                    if (stream_store_allowed) {
                        //uni_vmovntps(vmmword[reg_dst + reg_soff_nspc + offt],
                          //      Vmm(idx));
                        add_imm(X_TMP_0, reg_soff_nspc, offt);
    ...
    void var_channels() {
        ...
        L(ch_label);
        {
            ...
            //cmp(reg_coff, reg_coff_max);
            //jl(ch_label);
            add_imm(reg_coff, reg_coff, vlen);
            blt(reg_coff, reg_coff_max, ch_label);
        }
    }
    ...
    void forward_channels_nspc() {
        ...
        for (int ch_idx = max_ch_unroll; ch_idx > 0; --ch_idx) {
            L(ch_unroll_label[ch_idx]);
            {
                ...
                add(reg_ws, reg_ws, 2 * ch_blk_size, X_TMP_0);
