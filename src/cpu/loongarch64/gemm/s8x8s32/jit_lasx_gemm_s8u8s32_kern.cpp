    ...
    L_aligned(label_kernel_loop);
    {
        for (int h = 0; h < 4; h++) {
            for (int j = 0; j < max_unroll_n_; j++) {
                ...
                if (unroll_n > j) {
                    //vpbroadcastd(b, ptr[BO_ + 4 * j + 4 * h * unroll_n - offset_b_]);
...
void jit_lasx_gemm_s8u8s32_kern::generate() {
    ...
    //mov(C_, arg_c_);  // abi_param7 in loongarch
    //mov(LDC_, arg_ldc_);  // abi_param8 on loongarch
    ...
    //     vinsgr2vr_d(make_xmm(ones_), rax, 0);
    //     //vpbroadcastw(ones_, make_xmm(ones_));
    //     xvreplgr2vr_h(ones_, rax);
    // }
