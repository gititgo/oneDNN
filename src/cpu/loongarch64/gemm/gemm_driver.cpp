...
#include "cpu/loongarch64/gemm/gemm_partition.hpp"
#include "cpu/loongarch64/gemm/gemm_threading.hpp"
#include "cpu/loongarch64/gemm/gemm_utils.hpp"
#include "cpu/loongarch64/gemm/gemv_driver.hpp"

//#include "cpu/loongarch64/gemm/f32/jit_avx512_common_gemm_f32.hpp"
//#include "cpu/loongarch64/gemm/f32/jit_avx512_core_gemm_smalln_tn_f32_kern.hpp"
#include "cpu/loongarch64/gemm/f32/jit_lasx_gemm_f32.hpp"

//#include "cpu/loongarch64/gemm/s8x8s32/jit_avx512_core_gemv_s8x8s32.hpp"
...
int get_vector_length() {
    ...
    if (mayiuse(lasx))
        v_bytes = cpu_isa_traits<lasx>::vlen;
    else
        v_bytes = cpu_isa_traits<lsx>::vlen;
...
static inline bool nocopy_checker_lasx(const int nthr, const int transa,
    ...
    static const dim_t BM_NOCOPY_LASX = 64;
    //static const dim_t MN_NOCOPY_AVX2 = 128;
    static const dim_t MN_NOCOPY_LASX = 128;
    ...
    if (transb == no_trans) {
        if (m <= MN_NOCOPY_LASX && n <= MN_NOCOPY_LASX) return true;
        if (n <= nthr * N_NOTRANSB_PER_THR) return true;
        if (k <= nthr * K_NOTRANSB_PER_THR) return true;
        if (m <= BM_NOCOPY_LASX && n >= nthr * N_NOTRANSB_PER_THR) return true;
    } else {
        if (m <= MN_NOCOPY_LASX && n <= MN_NOCOPY_LASX) return true;
...
/* loongarch do not support 512 vector
...
static dnnl_status_t call_no_copy_sgemm(
    ...
    if (arg->packing == pack_type::none) {
        ...
        //else
            return jit_lasx_gemm_f32(nthrs, transa_char, transb_char, &arg->m,
...
static dnnl_status_t gemm_threading_driver(
    ...
    /* is useful or not in loongarch not sure
            ...
            for (; ithr < nthr_eff; ithr += nthr) {
                ...
                switch (thread_info.copy) {
                    ...
                    case copy_type::no_copy:
                        ...
                        //} else {
                            lasx_gemm_f32::sgemm_nocopy_driver(
                                    arg->transa == no_trans ? "N" : "T",
                                    arg->transb == no_trans ? "N" : "T", m, n,
                                    k, &arg->alpha, (float *)a, arg->lda,
                                    (float *)b, arg->ldb, &beta_eff,
...
dnnl_status_t gemm_driver(const char *transA, const char *transB,
    ...
    assert(IMPLICATION(is_int8, mayiuse(lsx)));
    ...
    assert(IMPLICATION(
            data_traits<a_type>::data_type == data_type::f32, mayiuse(lsx)));
    ...
    assert(IMPLICATION(force_nocopy, mayiuse(lasx)));
