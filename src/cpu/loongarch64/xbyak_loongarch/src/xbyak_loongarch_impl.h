/*******************************************************************************
 * Copyright 2020-2021 LOONGSON LIMITED
 *
...
/*
//this LSX encode is diff with LASX
template <typename T> 
void CodeGenerator::LsxFormatI8ELM(uint32_t op, uint32_t df, uint32_t idx, int32_t imm8, const T &rj, const T &rd ) {
  verifyIncRange(imm8, -1*(1<<7), ones(7), ERR_ILLEGAL_IMM_RANGE, true);

  uint32_t df2 = df & ones(2);
  uint32_t df_n = 0;
  ...
  switch(df2) {
    ...
    case 3:  //D: 00010|idx1
    { 
      verifyIncRange(off, 0, ones(1), ERR_ILLEGAL_IMM_RANGE, false);
  ...
  uint32_t code = concat({F(op, 24),F(df_n, 18), F(imm8&ones(8), 10), F(rj.getIdx(), 5), F(rd.getIdx(), 0)});
  dd(code);
}
*/
