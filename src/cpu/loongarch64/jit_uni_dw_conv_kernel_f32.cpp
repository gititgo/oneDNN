...
void jit_uni_dw_conv_fwd_kernel_f32<lasx>::add_tail_from_mem(Vmm &vmm_acc,
        Vmm &vmm_tmp, const Xbyak_loongarch::XReg &reg, int64_t offset, int load_size) {
...
//template struct jit_uni_dw_conv_fwd_kernel_f32<lsx>;

template <cpu_isa_t isa>
inline void jit_uni_dw_conv_bwd_data_kernel_f32<isa>::load_vmm(
        //Vmm &vmm, const Xbyak::Address &addr, bool tail) {
        Vmm &vmm, const Xbyak_loongarch::XReg &addr, int offset, bool tail) {
...
//template struct jit_uni_dw_conv_bwd_data_kernel_f32<lsx>;
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::load_xmm(
        ...
        Vmm &vmm, const Xbyak_loongarch::XReg &addr, int offset, bool compute_tail) {
    int ch_tail = jcp.oc_without_padding % simd_w_; // special case for SSE41
    int bytes
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::addps_xmm(Vmm &vmm_dst,
        ...
        Vmm &vmm_src, const Xbyak_loongarch::XReg &addr, int offset, bool compute_tail) {
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<lasx>::addps_xmm(
        Vmm &vmm_dst, Vmm &vmm_src, const Xbyak_loongarch::XReg &addr, int offset,
        bool compute_tail) {
    if (compute_tail) {
...
void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_single_ch_block_bias() {
    ...
    auto write_compute_bias = [&](bool is_last_ch) {
        ...
        //jnz(skip_load_bias, T_NEAR);
        //sub_d(X_TMP_0, reg_exec_flags, reg_exec_flags);
...
void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::deploy_ch_loop_bias() {
    ...
    //sub_d(X_TMP_1, reg_exec_flags, reg_exec_flags);
...
void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::generate() {
   ...
   /* bool set_kmask = isa > lasx && jcp.ch_tail > 0
            && (jcp.with_bias || is_layout_nxc());
    if (set_kmask) {
        // Prepare masks for tail
        //Reg32 reg_tmp_32 = reg_tmp.cvt32();
        //mov(reg_tmp_32, (1 << jcp.ch_tail) - 1);
        mov_imm(reg_tmp, (1 << jcp.ch_tail) - 1);
        //kmovw(k_ch_tail_mask, reg_tmp_32);
        ext_w_h(k_ch_tail_mask, reg_tmp);
    }*/
...
//template struct jit_uni_dw_conv_bwd_weights_kernel_f32<lsx>;
