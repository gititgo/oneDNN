...
private:
    ...
    void reduce_ymm_to_xmm(const Xbyak_loongarch::VReg &acc, const Xbyak_loongarch::VReg &tmp);

    void reduce_xmm_to_scalar(const Xbyak_loongarch::VReg &acc, 
            const Xbyak_loongarch::VReg &tmp, const std::size_t number_of_values_to_reduce
            = number_of_f32_in_xmm_);
  
    /*void reduce_zmm_to_ymm(const Xbyak::Xmm &acc, const Xbyak::Xmm &tmp);*/
    void reduce_ymm_to_scalar(const Xbyak_loongarch::VReg &acc, 
            const Xbyak_loongarch::VReg &tmp1,
            const Xbyak_loongarch::VReg &tmp2,
            const std::size_t number_of_values_to_reduce
            = number_of_f32_in_ymm_);
    void reduce_vmm_to_scalar(const Xbyak_loongarch::VReg &acc, 
            const Xbyak_loongarch::VReg &tmp1, const Xbyak_loongarch::VReg &tmp2, 
            const Xbyak_loongarch::VReg &tmp3, const std::size_t number_of_values_to_reduce
            = number_of_f32_in_ymm_);
   ...
   /* const Xbyak_loongarch::VReg vmm_tail_load_mask_ = Xbyak_loongarch::VReg(0);
    const Xbyak_loongarch::VReg vmm_tail_store_mask_ = Xbyak_loongarch::VReg(1);
    const Xbyak_loongarch::VReg vmm_zero_saturation_ = Xbyak_loongarch::VReg(2);
    const Xbyak_loongarch::VReg vmm_saturation_ubound_ = Xbyak_loongarch::VReg(3);
    const Xbyak_loongarch::VReg vmm_acc_ = Xbyak_loongarch::VReg(4);
    const Xbyak_loongarch::VReg vmm_tmp1_ = Xbyak_loongarch::VReg(5);
    const Xbyak_loongarch::VReg vmm_tmp2_ = Xbyak_loongarch::VReg(6);
    const Xbyak_loongarch::VReg vmm_tmp3_ = Xbyak_loongarch::VReg(7);
    const Xbyak_loongarch::VReg vmm_tmp4_ = Xbyak_loongarch::VReg(8);*/
    ...
    const Xbyak_loongarch::XReg &k_tail_load_mask_ = t7;
    const Xbyak_loongarch::XReg &k_tail_store_mask_ = t8;  
    ...
    const Xbyak_loongarch::XReg &reg_work_ = t0;//rax;
    const Xbyak_loongarch::XReg &reg_src_ = t1;//rbx;
    const Xbyak_loongarch::XReg &reg_dst_ = t4;//rdx;
    const Xbyak_loongarch::XReg &reg_param_ = abi_param1;
    const Xbyak_loongarch::XReg &reg_tmp_ = abi_not_param1;

    //static constexpr bool is_zmm_ = std::is_same<Vmm, Xbyak::Zmm>::value;
    static constexpr bool is_ymm_ = std::is_same<Vmm, Xbyak_loongarch::XVReg>::value;
