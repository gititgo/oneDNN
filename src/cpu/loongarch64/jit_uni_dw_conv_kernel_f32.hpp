...
private:
    ...
    //using reg64_t = const Xbyak_loongarch::XReg;
    //using mask_t = const Xbyak_loongarch::XReg;

    const int vlen = cpu_isa_traits<isa>::vlen;

    // dw convolution
    Xbyak_loongarch::XReg reg_input = a3;//r8;
    Xbyak_loongarch::XReg aux_reg_input = t1;//r9;
    Xbyak_loongarch::XReg reg_kernel = t3;//r10;
    Xbyak_loongarch::XReg aux_reg_kernel = t4;//r11;
    Xbyak_loongarch::XReg reg_ch_blocks = t5;//r12;
    Xbyak_loongarch::XReg reg_output = a4;//r13;
    Xbyak_loongarch::XReg reg_bias = a5;//r14;
    Xbyak_loongarch::XReg reg_kh = a6;//r15;
    Xbyak_loongarch::XReg iter_kh = t0;//rax;
    Xbyak_loongarch::XReg reg_oi = t6;//rbx;
    Xbyak_loongarch::XReg aux_reg_ch_blocks = t7;//rsi;
    // fused convolution
    Xbyak_loongarch::XReg reg_input_buffer_ptr = t8;//rdx;
    Xbyak_loongarch::XReg aux_reg_input_buffer_ptr = a7;//rbp;
    Xbyak_loongarch::XReg reg_iw_offset = reg_input; //Hack: clear reg_input early in kernel

    Xbyak_loongarch::XReg reg_tail = t0;//rax;
    //Xbyak_loongarch::XReg k_oc_tail_mask = t8;//Xbyak_loongarch::XReg(2); //Xbyak::Opmask(2);
    ...
    int max_repeats() { return jcp.isa == lsx ? 2 : 1; }//return jcp.isa == sse41 ? 2 : 1; }
    ...
    void load_tail(
            ...
            Vmm &vmm, const Xbyak_loongarch::XReg &reg, int64_t offset, int load_size);
    //void add_tail_from_mem(Vmm &vmm_acc, Vmm &vmm_tmp, const Xbyak::Reg64 &reg,
    void add_tail_from_mem(Vmm &vmm_acc, Vmm &vmm_tmp, const Xbyak_loongarch::XReg &reg,
            int64_t offset, int load_size);
    void store_tail(
            //Vmm &vmm, const Xbyak::Reg64 &reg, int64_t offset, int store_size);
            Vmm &vmm, const Xbyak_loongarch::XReg &reg, int64_t offset, int store_size);
...
private:
    ...
    reg64_t aux1_reg_ddst = abi_not_param1;     //?abi_not_param1未知是什么，等头文件叭
    ...
    Xbyak_loongarch::XReg k_ch_tail_mask = Xbyak_loongarch::XReg(1);


    //void load_vmm(Vmm &vmm, const Xbyak::Address &addr, bool tail);
    //void store_vmm(Vmm &vmm, const Xbyak::Address &addr, bool tail);
    void load_vmm(Vmm &vmm, const Xbyak_loongarch::XReg &addr, int offset, bool tail);
    void store_vmm(Vmm &vmm, const Xbyak_loongarch::XReg &addr, int offset, bool tail);
...
private:
    ...
    using reg64_t = const Xbyak_loongarch::XReg;  //const Xbyak::Reg64;
    ...
    Xbyak_loongarch::XReg reg_exec_flags = t0;
    ...
    void addps_xmm(Vmm &vmm_dst, Vmm &vmm_src, const Xbyak_loongarch::XReg &addr,
            int offset, bool compute_tail);
    void load_xmm(
            //Vmm &vmm, const Xbyak::Address &addr, bool compute_tail = false);
            Vmm &vmm, const Xbyak_loongarch::XReg &addr,
