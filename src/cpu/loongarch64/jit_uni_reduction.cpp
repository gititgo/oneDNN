...
status_t jit_uni_reduction_t::get_proper_kernel(
   ...
   /* if (is_superset(conf.isa, avx512_common))
    ...
    else if (is_superset(conf.isa, lasx)) {*/
    if (conf.isa == lasx) {
        ...
        if (is_src_i8 || is_dst_i8)
            return safe_ptr_assign(kernel_,
                    new jit_uni_reduction_kernel_t<Xbyak_loongarch::VReg>(conf, dst_md));
    
        return safe_ptr_assign(kernel_,
                new jit_uni_reduction_kernel_t<Xbyak_loongarch::XVReg>(conf, dst_md));
    //} else if (is_superset(conf.isa, lsx))
