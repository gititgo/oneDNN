...
struct jit_uni_i8i8_pooling_fwd_ker_t : public jit_generator {
    ...
    // In case of lasx with data type i8 we need to use
...
/* do not need load_vreg_mask_q because loongarch do not have vpalignr
template <>
void jit_uni_i8i8_pooling_fwd_ker_t<lsx>::load_vreg_mask_q(int ll) {};

template <>
void jit_uni_i8i8_pooling_fwd_ker_t<lasx>::load_vreg_mask_q(int ll) {
...
void jit_uni_i8i8_pooling_fwd_ker_t<lasx>::load_src_max_op(
    ...
    /* code is too complicated so loongarch use load_bytes
    if (masked) {
        if (jpp.src_dt == s32) {
            ...
            const uint8_t shift = cpu_isa_traits<lasx>::vlen - jpp.c_tail;
...
void jit_uni_i8i8_pooling_fwd_ker_t<lasx>::load_src_avg_op(
    ...
    auto load_i8 = [&](bool is_signed, const Vmm &vr_src) {
        ...
        if (masked) {
            ...
            const uint8_t shift = cpu_isa_traits<lasx>::vlen
...
void jit_uni_i8i8_pooling_fwd_ker_t<lasx>::store_dst_max_op(
    ...
    /* code is too complicated so loongarch use store_bytes and uni_xvst
...
void jit_uni_i8i8_pooling_fwd_ker_t<lasx>::store_dst_avg_op(
    ...
    /* code is too complicated so loongarch use store_bytes
...
void jit_uni_i8i8_pooling_fwd_ker_t<isa>::compute_avg_step(
    ...
    const injector_utils::register_preserve_guard_t reg_guard(this,
            jpp.with_binary
                    ? std::initializer_list<Xbyak_loongarch::XReg> {reg_tmp_postops}
...
void jit_uni_i8i8_pooling_fwd_ker_t<lasx>::init_mask() {
    /* loongarch do not need init_mask
    using namespace data_type;
    using cpu_isa = cpu_isa_traits<lasx>;
