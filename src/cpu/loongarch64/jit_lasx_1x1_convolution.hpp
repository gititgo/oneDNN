...
#ifndef CPU_LOONGARCH64_JIT_LASX_1X1_CONVOLUTION_HPP
#define CPU_LOONGARCH64_JIT_LASX_1X1_CONVOLUTION_HPP
...
#include "cpu/loongarch64/cpu_reducer.hpp"
...
struct jit_lasx_1x1_convolution_fwd_t : public primitive_t {
    ...
    struct pd_t : public cpu_convolution_fwd_pd_t {
        ...
        DECLARE_COMMON_PD_T(JIT_IMPL_NAME_HELPER("jit_1x1:", jcp_.isa, ""),
                jit_lasx_1x1_convolution_fwd_t);
        ...
        status_t init(engine_t *engine) {
            ...
            status_t status = jit_lasx_1x1_conv_kernel_f32::init_conf(
            ...
            jit_lasx_1x1_conv_kernel_f32::init_scratchpad(scratchpad, jcp_);
    ...
    protected:
        ...
        status_t copy(const pd_t &other) {
            ...
            if (other.dw_conv_pd_) {
                ...
                if (jcp_.isa == lasx) {
                    jcp_dw_ = &(static_cast<dw_pd_t<lasx> *>(dw_conv_pd_.get())
                                        ->jcp_);
                } //else { // lsx
                //    jcp_dw_ = &(static_cast<dw_pd_t<lsx> *>(dw_conv_pd_.get())
                //                        ->jcp_);
                //}
            }
        ...
        status_t depthwise_po_init(engine_t *engine) {
            ...
            if (jcp_1x1.isa == lasx) {
                std::unique_ptr<dw_pd_t<lasx>> fusable_pd(
                        new dw_pd_t<lasx>(&cd_dw, &attr_dw, nullptr));
                CHECK(fusable_pd->init(engine));
                jcp_dw = &(fusable_pd->jcp_);
                dw_conv_pd_ = std::move(fusable_pd);
            } //else {
                // Special case for this primitive, as we dont have dw<lasx>.
                // In this case fuse with lsx depthwise conv
                // NOTE: Currently dw f32 kernel is similar for all ISA and can
                // be fused regardless of ISA if inter-connecting md_ matches.
            //    std::unique_ptr<dw_pd_t<lsx>> fusable_pd(
            //            new dw_pd_t<lsx>(&cd_dw, &attr_dw, nullptr));
            //    CHECK(fusable_pd->init(engine));
            //    jcp_dw = &(fusable_pd->jcp_);
            //    dw_conv_pd_ = std::move(fusable_pd);
            //}
            ...
            if (jcp_1x1.isa == lasx)
                dw_conv_kernel_t<lasx>::init_scratchpad(dw_scratchpad, *jcp_dw);
            //else
            //    dw_conv_kernel_t<lsx>::init_scratchpad(
    ...
    jit_lasx_1x1_convolution_fwd_t(const pd_t *apd) : primitive_t(apd) {}

    status_t init(engine_t *engine) override {
        CHECK(safe_ptr_assign(kernel_,
                new jit_lasx_1x1_conv_kernel_f32(
                        pd()->jcp_, *pd()->attr(), *pd()->dst_md(0))));
        CHECK(kernel_->create_kernel());
        CHECK(init_rtus_driver<lasx>(this));
        if (pd()->jcp_.with_dw_conv) {
            auto &isa = pd()->jcp_.isa;

            if (isa == lasx) {
                CHECK(safe_ptr_assign(kernel_dw_lasx,
                        new dw_conv_kernel_t<lasx>(
                                *(pd()->jcp_dw_), *pd()->dst_md(0))));
                CHECK(kernel_dw_lasx->create_kernel());
            } //else {
            //    CHECK(safe_ptr_assign(kernel_dw_lsx,
            //            new dw_conv_kernel_t<lsx>(
            //                    *(pd()->jcp_dw_), *pd()->dst_md(0))));
            //    CHECK(kernel_dw_lsx->create_kernel());
            //}
        }
...
private:
    ...
    std::unique_ptr<jit_lasx_1x1_conv_kernel_f32> kernel_;
    std::unique_ptr<rtus_driver_t<lasx>> rtus_driver_;

    template <cpu_isa_t isa>
    using dw_conv_kernel_t = jit_uni_dw_conv_fwd_kernel<isa, data_type::f32>;

    std::unique_ptr<dw_conv_kernel_t<lasx>> kernel_dw_lasx;
    //std::unique_ptr<dw_conv_kernel_t<lsx>> kernel_dw_lsx;
};

struct jit_lasx_1x1_convolution_bwd_data_t : public primitive_t {
    struct pd_t : public cpu_convolution_bwd_data_pd_t {
        ...
        DECLARE_COMMON_PD_T(JIT_IMPL_NAME_HELPER("jit_1x1:", lasx, ""),
                jit_lasx_1x1_convolution_bwd_data_t);
        ...
        status_t init(engine_t *engine) {
            ...
            status_t status = jit_lasx_1x1_conv_kernel_f32::init_conf(jcp_,
    ...
    jit_lasx_1x1_convolution_bwd_data_t(const pd_t *apd) : primitive_t(apd) {}
...
struct jit_lasx_1x1_convolution_bwd_weights_t : public primitive_t {
    struct pd_t : public cpu_convolution_bwd_weights_pd_t {
        ...
        DECLARE_COMMON_PD_T(JIT_IMPL_NAME_HELPER("jit_1x1:", lasx, ""),
                jit_lasx_1x1_convolution_bwd_weights_t);
    ...
    jit_lasx_1x1_convolution_bwd_weights_t(const pd_t *apd)
        : primitive_t(apd) {}
