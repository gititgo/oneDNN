...
void jit_lasx_conv_fwd_kernel_f32::width_blk_step(
    ...
    L(kh_loop);
    {
        if (jcp.kw >= 5 && pad_l == 0 && pad_r == 0) {
            ...
            //safe_add(aux_reg_kernel, get_kernel_offset(0, kw, 0), reg_long_offt);
            add_imm(aux_reg_kernel,
                    aux_reg_kernel,
...
void jit_lasx_conv_bwd_data_kernel_f32::compute_loop(
    ...
    if (jcp.ndims == 5) {
        //sub(aux_reg_dst_d, get_ddst_offset(0, (jcp.dilate_d + 1) * jcp.oh * ow, 0));
    ...
    if (one_of(jcp.ndims, 3, 4)) {
        ...
        if (oc_tail){
            //sub(reg_reduce_work, jcp.oc_block);
    ...
    L(kh_loop);
    {
        ...
        L(ic_block_loop);
        {
            if (jcp.l_pad != 0) {
                ...
                //add(reg_input, get_input_offset(0, ur_w * stride_w - jcp.l_pad));
                add_imm(reg_input, reg_input, 
