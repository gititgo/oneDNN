...
jit_io_helper_t<Vmm>::jit_io_helper_t(jit_generator *host, const cpu_isa_t &isa,
    ...
    static constexpr bool is_xmm = std::is_same<Vmm, Xbyak_loongarch::VReg>::value;
...
void jit_io_helper_t<Vmm>::prepare_opmask(
        ...
        const std::size_t how_many_bits_to_set, const Xbyak_loongarch::XReg &reg_tmp,
        //const Xbyak::Opmask &mask) {
        const Xbyak_loongarch::XReg &mask) {
...
void jit_io_helper_t<Vmm>::prepare_vmm_mask(
        ...
        const Xbyak_loongarch::XReg &reg_tmp, const Vmm &mask) {
...
void jit_io_helper_t<Vmm>::prepare_i8_data_to_store(const Vmm &i8_vmm) {
    ...
    if (isa_ == lasx) {
        ...
        const auto src_ymm = Xbyak_loongarch::XVReg(i8_vmm.getIdx());
...
//        const bool tail) {
void jit_io_helper_t<Xbyak_loongarch::XVReg>::emu_gather(const Xbyak_loongarch::XReg &src_reg,
        const Xbyak_loongarch::XVReg &indices_vmm, const Xbyak_loongarch::XVReg &dst_vmm,
        const bool tail) {
    assert(gather_conf_.has_value() && "Config for loading with the use of gather instruction is not set.");
    assert(gather_conf_->vmm_tmp_idx_.has_value()
            && "Temporary vreg is not set.");
    ...
    //const Xbyak::Xmm xmm_tmp = Xbyak::Xmm(gather_conf_->full_vmm_mask_idx_);
    const Xbyak_loongarch::VReg xmm_tmp = Xbyak_loongarch::VReg(gather_conf_->full_vmm_mask_idx_);
    //const Xbyak::Xmm xmm_dst = Xbyak::Xmm(*gather_conf_->vmm_tmp_idx_);
    const Xbyak_loongarch::VReg xmm_dst = Xbyak_loongarch::VReg(*gather_conf_->vmm_tmp_idx_);
...
//        const bool tail) {
void jit_io_helper_t<Xbyak_loongarch::VReg>::emu_gather(const Xbyak_loongarch::XReg &src_reg,
        const Xbyak_loongarch::VReg &indices_vmm, const Xbyak_loongarch::VReg &dst_vmm,
        const bool tail) {
    assert(gather_conf_.has_value() && "Config for loading with the use of gather instruction is not set.");
    assert(IMPLICATION(tail, tail_conf_.has_value())
            && "Config for tail processing is not set.");
...
void jit_io_helper_t<Vmm>::gather(const Xbyak_loongarch::XReg &src_reg,
...
void jit_io_helper_t<Vmm>::load(const Xbyak_loongarch::XReg &src_addr, const int32_t offset,
...
void jit_io_helper_t<Vmm>::load_byte_by_byte(const Xbyak_loongarch::XReg &src_addr,
...
//        const Xbyak::Address &src_addr, const Vmm &dst_vmm) {
void jit_io_helper_t<Vmm>::load_i8(const Xbyak_loongarch::XReg &src_addr,
    ...
    if (data_type_ == data_type::s8)
        host_->vext2xv_w_b(Xbyak_loongarch::XVReg(dst_vmm.getIdx()), Xbyak_loongarch::XVReg(dst_vmm.getIdx()));
    else
        host_->vext2xv_wu_bu(Xbyak_loongarch::XVReg(dst_vmm.getIdx()), Xbyak_loongarch::XVReg(dst_vmm.getIdx()));
...
void jit_io_helper_t<Vmm>::store(const Vmm &src_raw_vmm, const Xbyak_loongarch::XReg &dst_raw_addr,
...
void jit_io_helper_t<Vmm>::store_byte_by_byte(const Vmm &src_vmm,
        const Xbyak_loongarch::XReg &dst_addr, const int32_t offset, const int store_size) {
...
void jit_io_helper_t<Vmm>::store_f32(const Vmm &src_vmm, 
        const Xbyak_loongarch::XReg &dst_addr, const int32_t offset, const bool tail) {
...
void jit_io_helper_t<Vmm>::store_i8(const Vmm &src_vmm, 
        const Xbyak_loongarch::XReg &dst_addr, const int32_t offset) {
    //if (!is_superset(isa_, avx512_common)) {
    //    static constexpr bool is_ymm = std::is_same<Vmm, Xbyak::Ymm>::value;
...
void jit_io_helper_t<Vmm>::convert_to_f32(const Vmm &dst_vmm,
        const Xbyak_loongarch::VReg &src_vmm, const data_type_t src_data_type) {
...
//        const Xbyak::Address &src_addr, const Vmm &dst_vmm) {
void jit_io_helper_t<Vmm>::broadcast(const Xbyak_loongarch::XReg &src_addr,
        const int32_t offset, const Vmm &dst_vmm) {
    switch (data_type_) {
...
template class jit_io_helper_t<Xbyak_loongarch::XVReg>;
template class jit_io_helper_t<Xbyak_loongarch::VReg>;
...
template class jit_io_multi_dt_helper_t<Xbyak_loongarch::XVReg>;
template class jit_io_multi_dt_helper_t<Xbyak_loongarch::VReg>;
