...
struct rhs_arg_static_params_t {
    ...
    Xbyak_loongarch::XReg tail_opmask;
    bool use_exact_tail_scalar_bcast;
...
struct rhs_arg_dynamic_params_t {
    ...
    std::map<int, Xbyak_loongarch::XReg> vmm_idx_to_out_off_oprnd;
    ...
    std::map<int, Xbyak_loongarch::XReg> vmm_idx_to_oc_off_oprnd;
    ...
    std::map<int, Xbyak_loongarch::XReg> vmm_idx_to_sp_off_oprnd;
...
private:
    ...
    void append_offset_from_operand(
            ...
            const std::map<int, Xbyak_loongarch::XReg> &vmm_idx_to_elem_addr_off,
    ...
    //typename std::enable_if<!(std::is_same<T, Xbyak::Zmm>::value
    //        || std::is_same<T, Xbyak::Address>::value)>::type
    void execute_cmp_binary(const Vmm &dst) const;
    template <typename T>
