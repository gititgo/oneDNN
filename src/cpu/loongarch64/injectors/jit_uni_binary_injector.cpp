...
static_params_t::static_params_t(const Xbyak_loongarch::XReg &param1,
        const bcast_set_t &supported_strategy_set,
...
rhs_arg_static_params_t::rhs_arg_static_params_t(
        ...
        std::size_t rhs_dt_helper_vmm_idx, const Xbyak_loongarch::XReg &rhs_addr_reg,
    ...
    : rhs_arg_static_params_t(rhs_dt_helper_vmm_idx, rhs_addr_reg,
            ...
            abi_param_offset, dst_d, tail_size, Xbyak_loongarch::XReg(31),
...
rhs_arg_static_params_t::rhs_arg_static_params_t(
        ...
        const Xbyak_loongarch::XReg &tail_opmask, bool use_exact_tail_scalar_bcast)
    : rhs_arg_static_params_t(rhs_dt_helper_vmm_idx, rhs_addr_reg,
            rhs_helper_reg, preserve_gpr_helpers, preserve_vmm_helper,
...
rhs_arg_static_params_t::rhs_arg_static_params_t(
        ...
        const Xbyak_loongarch::XReg &tail_opmask, const Xbyak_loongarch::XReg &reg_tail_size,
        bool use_exact_tail_scalar_bcast)
    : rhs_arg_static_params_t(rhs_dt_helper_vmm_idx, rhs_addr_reg,
            rhs_helper_reg, preserve_gpr_helpers, preserve_vmm_helper,
            abi_param_offset, dst_d, tail_size, tail_opmask,
...
void jit_uni_binary_injector_t<isa, Vmm>::compute_vector_range(
    ...
    const injector_utils::register_preserve_guard_t register_guard {host_,
            (rhs_arg_static_params_.preserve_gpr_helpers
                            ...
                            ? std::initializer_list<Xbyak_loongarch::XReg>(
            ...
            (rhs_arg_static_params_.preserve_vmm_helper && dt_helper_vmm_needed
                            ...
                            ? std::initializer_list<Xbyak_loongarch::VReg>({Vmm(vmm_hint).getIdx()})
                            //: std::initializer_list<Xbyak::Xmm>())};
                            : std::initializer_list<Xbyak_loongarch::VReg>())};

    bool vmm0_was_preserved = false;
    static const Vmm zero_vmm(0);

    //Xbyak::Address rhs_arg_addr(0);
    Xbyak_loongarch::Address rhs_arg_addr(Xbyak_loongarch::XReg(0), 0);
...
Xbyak_loongarch::Address jit_uni_binary_injector_t<isa, Vmm>::prepare_rhs_arg_addr(
...
void jit_uni_binary_injector_t<isa, Vmm>::append_offset_from_operand(
        ...
        const std::map<int, Xbyak_loongarch::XReg> &vmm_idx_to_elem_operand_off,
        //int vmm_idx, const Xbyak::Reg64 &addr_reg, const Xbyak::Reg64 &tmp_reg,
        int vmm_idx, const Xbyak_loongarch::XReg &addr_reg, const Xbyak_loongarch::XReg &tmp_reg,
...
void jit_uni_binary_injector_t<isa, Vmm>::append_value_offset(
        ...
        const Xbyak_loongarch::XReg &addr_reg, std::size_t elem_size_bytes) const {
...
void jit_uni_binary_injector_t<isa, Vmm>::execute_broadcast(
        ...
        const Xbyak_loongarch::Address &rhs_addr, bool with_tail) const {
    if (with_tail)
...
void jit_uni_binary_injector_t<isa, Vmm>::load_rhs(
        ...
        const Xbyak_loongarch::Address &rhs_addr, const tail_lode_mode_t tail_load_mode,
...
Xbyak_loongarch::Address jit_uni_binary_injector_t<isa, Vmm>::remove_bcast_bit(
    ...
    return Xbyak_loongarch::Address(rhs_addr.getXReg(), rhs_addr.getOffset());
}
...
void jit_uni_binary_injector_t<lsx, Xbyak_loongarch::VReg>::cvt_to_f32(
        //const Xbyak::Xmm &tmp_vmm) const {
        const Xbyak_loongarch::VReg &tmp_vmm) const {
...
void jit_uni_binary_injector_t<isa, Vmm>::execute_broadcast_s8u8_no_tail(
    ...
    const Xbyak_loongarch::VReg xmm(tmp_vmm.getIdx());
    const Xbyak_loongarch::XVReg ymm(tmp_vmm.getIdx());
...
struct helper_broadcast_s8u8_t<lasx, Vmm> {
    static void execute_broadcast_s8u8_no_tail(jit_generator *host,
            ...
            const Vmm &tmp_vmm, const Xbyak_loongarch::Address &rhs_addr,
        ...
        const Xbyak_loongarch::XReg tmp_reg32 = Xbyak_loongarch::XReg(rhs_helper_reg_idx);
        //const auto tmp_xmm = Xbyak::Xmm(tmp_vmm.getIdx());
        const auto tmp_xmm = Xbyak_loongarch::VReg(tmp_vmm.getIdx());
        const auto tmp_ymm = Xbyak_loongarch::XVReg(tmp_vmm.getIdx());
        ...
        host->vshuf4i_h(Xbyak_loongarch::VReg(31), tmp_xmm, 0); // low d use low h
        host->vextrins_d(tmp_xmm, Xbyak_loongarch::VReg(31), 0); // high d keep unchanged
        if (data_type == data_type::s8)
...
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::XVReg>::execute_broadcast_s8u8_no_tail(
        //const data_type_t &data_type, const Xbyak::Ymm &tmp_vmm,
        const data_type_t &data_type, const Xbyak_loongarch::XVReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
    ...
    helper_broadcast_s8u8_t<lasx, Xbyak_loongarch::XVReg>::execute_broadcast_s8u8_no_tail(
...
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::VReg>::execute_broadcast_s8u8_no_tail(
        //const data_type_t &data_type, const Xbyak::Xmm &tmp_vmm,
        const data_type_t &data_type, const Xbyak_loongarch::VReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
    ...
    helper_broadcast_s8u8_t<lasx, Xbyak_loongarch::VReg>::execute_broadcast_s8u8_no_tail(
...
void jit_uni_binary_injector_t<lsx,
        ...
        Xbyak_loongarch::VReg>::execute_broadcast_s8u8_no_tail(const data_type_t
                                                            &data_type,
        //const Xbyak::Xmm &tmp_vmm, const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::VReg &tmp_vmm, const Xbyak_loongarch::Address &rhs_addr) const {
    ...
    if (data_type == data_type::s8 || data_type == data_type::u8) {
        ...
        const Xbyak_loongarch::XReg tmp_reg32 = Xbyak_loongarch::XReg(tmp_reg64_idx);
        const Xbyak_loongarch::XVReg tmp_ymm = Xbyak_loongarch::XVReg(tmp_vmm.getIdx());
        ...
        host_->vshuf4i_h(Xbyak_loongarch::VReg(31), tmp_vmm, 0); // low d use low h
        host_->vextrins_d(tmp_vmm, Xbyak_loongarch::VReg(31), 0); // high d keep unchanged
        if (data_type == data_type::s8)
...
static void load_tail_avx(jit_generator *host, std::size_t ymm_idx,
    ...
    if (should_load_lower_half) {
        ...
        const auto tmp_xmm = Xbyak_loongarch::VReg(ymm_idx);
        ...
        if (ymm_upper_half_op_data_size) {
            ...
            const auto tmp_ymm = Xbyak_loongarch::XVReg(ymm_idx);
            //host->vinsertf128(tmp_ymm, tmp_ymm, host->ptr[host->rsp], 1);
            host->uni_xvld(Xbyak_loongarch::XVReg(31), host->sp, 0);
            host->xvpermi_q(tmp_ymm, Xbyak_loongarch::XVReg(31), 0x02);
...
static void execute_broadcast_f32_tail_avx(jit_generator *host,
        ...
        const Xbyak_loongarch::XVReg &vmm, const Xbyak_loongarch::Address &rhs_addr,
        std::size_t tail_size) {

    const auto vmm_idx = vmm.getIdx();
    //const auto tmp_xmm = Xbyak::Xmm(vmm_idx);
    const auto tmp_xmm = Xbyak_loongarch::VReg(vmm_idx);
...
static void execute_broadcast_f32_tail_avx(jit_generator *host,
        ...
        const Xbyak_loongarch::VReg &vmm, const Xbyak_loongarch::Address &rhs_addr,
        std::size_t tail_size) {
...
template <>
//void jit_uni_binary_injector_t<avx2, Xbyak::Ymm>::execute_broadcast_tail(
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::XVReg>::execute_broadcast_tail(
        //const dnnl_data_type_t &data_type, const Xbyak::Ymm &tmp_vmm,
        const dnnl_data_type_t &data_type, const Xbyak_loongarch::XVReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
    const auto &tail_size = rhs_arg_static_params_.tail_size;
    //helper_bcast_tail_t<avx2, Xbyak::Ymm>::execute_broadcast_tail(
    helper_bcast_tail_t<lasx, Xbyak_loongarch::XVReg>::execute_broadcast_tail(
            host_, tail_size, data_type, tmp_vmm, rhs_addr);
}

template <>
//void jit_uni_binary_injector_t<avx2, Xbyak::Xmm>::execute_broadcast_tail(
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::VReg>::execute_broadcast_tail(
        //const dnnl_data_type_t &data_type, const Xbyak::Xmm &tmp_vmm,
        const dnnl_data_type_t &data_type, const Xbyak_loongarch::VReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
    const auto &tail_size = rhs_arg_static_params_.tail_size;
    //helper_bcast_tail_t<avx2, Xbyak::Xmm>::execute_broadcast_tail(
    helper_bcast_tail_t<lasx, Xbyak_loongarch::VReg>::execute_broadcast_tail(
            host_, tail_size, data_type, tmp_vmm, rhs_addr);
}

/* in loongarch only have lasx 
template <>
void jit_uni_binary_injector_t<avx, Xbyak::Ymm>::execute_broadcast_tail(
        const dnnl_data_type_t &data_type, const Xbyak::Ymm &tmp_vmm,
        const Xbyak::Address &rhs_addr) const {
...
template <>
void jit_uni_binary_injector_t<avx, Xbyak::Xmm>::execute_broadcast_tail(
        const dnnl_data_type_t &data_type, const Xbyak::Xmm &tmp_vmm,
        const Xbyak::Address &rhs_addr) const {
...
template <>
//void jit_uni_binary_injector_t<sse41, Xbyak::Xmm>::execute_broadcast_tail(
void jit_uni_binary_injector_t<lsx, Xbyak_loongarch::VReg>::execute_broadcast_tail(
        //const dnnl_data_type_t &data_type, const Xbyak::Xmm &tmp_vmm,
        const dnnl_data_type_t &data_type, const Xbyak_loongarch::VReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
...
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::XVReg>::load_rhs_i8_no_tail(
        //const dnnl_data_type_t &data_type, const Xbyak::Ymm &tmp_vmm,
        const dnnl_data_type_t &data_type, const Xbyak_loongarch::XVReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
    ...
    host_->uni_xvld(Xbyak_loongarch::XVReg(31), host_->sp, 0);
    host_->xvpermi_q(tmp_vmm, Xbyak_loongarch::XVReg(31), 0x02);
    restore_stack(host_, tmp_xmm);
}

/* remove this function because we cannot exec 512bit
template <cpu_isa_t isa, typename Vmm>
...
void jit_uni_binary_injector_t<isa, Vmm>::load_rhs_tail_dynamically_with_gpr(
    ...
    const bool is_ymm = std::is_same<Vmm, Xbyak_loongarch::XVReg>::value;
    //const Xbyak::Reg64 &reg_addr = rhs_arg_static_params_.rhs_addr_reg;
    const Xbyak_loongarch::XReg &reg_addr = rhs_arg_static_params_.rhs_addr_reg;
    //const Xbyak::Reg64 &reg_tmp = rhs_arg_static_params_.rhs_helper_reg;
    const Xbyak_loongarch::XReg &reg_tmp = rhs_arg_static_params_.rhs_helper_reg;
    //const Xbyak::Reg64 &reg_tail_size = rhs_arg_static_params_.reg_tail_size;
    const Xbyak_loongarch::XReg &reg_tail_size = rhs_arg_static_params_.reg_tail_size;
    //const Xbyak::Xmm x = Xbyak::Xmm(tmp_vmm.getIdx());
    const Xbyak_loongarch::VReg x = Xbyak_loongarch::VReg(tmp_vmm.getIdx());
    //const Xbyak::Ymm y = Xbyak::Ymm(tmp_vmm.getIdx());
    const Xbyak_loongarch::XVReg y = Xbyak_loongarch::XVReg(tmp_vmm.getIdx());
...
struct helper_load_tail_t<lasx, Vmm> {
    static void load_rhs_tail_statically(jit_generator *host,
            ...
            const size_t tail_size, const Xbyak_loongarch::XReg &rhs_addr_reg,
            const dnnl_data_type_t &data_type, const Vmm &tmp_vmm,
...
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::XVReg>::load_rhs_tail_statically(
        //const dnnl_data_type_t &data_type, const Xbyak::Ymm &tmp_vmm,
        const dnnl_data_type_t &data_type, const Xbyak_loongarch::XVReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
    ...
    helper_load_tail_t<lasx, Xbyak_loongarch::XVReg>::load_rhs_tail_statically(
...
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::VReg>::load_rhs_tail_statically(
        //const dnnl_data_type_t &data_type, const Xbyak::Xmm &tmp_vmm,
        const dnnl_data_type_t &data_type, const Xbyak_loongarch::VReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
    ...
    helper_load_tail_t<lasx, Xbyak_loongarch::VReg>::load_rhs_tail_statically(
            host_, tail_size, rhs_addr_reg, data_type, tmp_vmm, rhs_addr);
}

/* loongarch already have lasx so no need avx
template <>
...
void jit_uni_binary_injector_t<lsx, Xbyak_loongarch::VReg>::load_rhs_tail_statically(
        //const dnnl_data_type_t &data_type, const Xbyak::Xmm &tmp_vmm,
        const dnnl_data_type_t &data_type, const Xbyak_loongarch::VReg &tmp_vmm,
        //const Xbyak::Address &rhs_addr) const {
        const Xbyak_loongarch::Address &rhs_addr) const {
...
/* loongarch do not support 512bit vector
...
void jit_uni_binary_injector_t<isa, Vmm>::execute_binary(alg_kind_t binary_alg,
    ...
    if (std::is_same<T, Xbyak_loongarch::Address>::value) {
        const Xbyak_loongarch::Address &addr = (const Xbyak_loongarch::Address&)rhs;
        host_->uni_xvld(Vmm(31), addr.getXReg(), addr.getOffset());
    } else if (std::is_same<T, Xbyak_loongarch::XVReg>::value) {
        const Xbyak_loongarch::XVReg &xvreg = (const Xbyak_loongarch::XVReg&)rhs;
        host_->uni_bsll_v(Xbyak_loongarch::XVReg(31), xvreg, 0);
    } else if (std::is_same<T, Xbyak_loongarch::VReg>::value ) {
        const Xbyak_loongarch::VReg &vreg = (const Xbyak_loongarch::VReg&)rhs;
        host_->uni_bsll_v(Xbyak_loongarch::VReg(31), vreg, 0);
    }
...
struct helper_binary_t<lasx, Vmm> {
    ...
    static void execute_binary(jit_generator *host, F execute_cmp_binary,
        ...
        if (std::is_same<T, Xbyak_loongarch::Address>::value) {
            ...
            const Xbyak_loongarch::XVReg &xvreg = (const Xbyak_loongarch::XVReg&)rhs;
            host->uni_bsll_v(Xbyak_loongarch::XVReg(31), xvreg, 0);
        } else if (std::is_same<T, Xbyak_loongarch::VReg>::value ) {
            const Xbyak_loongarch::VReg &vreg = (const Xbyak_loongarch::VReg&)rhs;
            host->uni_bsll_v(Xbyak_loongarch::VReg(31), vreg, 0);
        }
        switch (binary_alg) {
            //case alg_kind::binary_add: host_->uni_vaddps(dst, lhs, rhs); break;
...
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::XVReg>::execute_binary(
        //alg_kind_t binary_alg, const Xbyak::Ymm &dst, const Xbyak::Ymm &lhs,
        alg_kind_t binary_alg, const Xbyak_loongarch::XVReg &dst, const Xbyak_loongarch::XVReg &lhs,
        const T &rhs) const {

    const auto execute_cmp_binary_lam
            //= [this](const Xbyak::Ymm &dst, const Xbyak::Ymm &lhs, const T &rhs,
            = [this](const Xbyak_loongarch::XVReg &dst) {
                  this->execute_cmp_binary(dst);
              };
    //helper_binary_t<avx, Xbyak::Ymm>::execute_binary<T>(
    helper_binary_t<lasx, Xbyak_loongarch::XVReg>::execute_binary<T>(
...
void jit_uni_binary_injector_t<lasx, Xbyak_loongarch::VReg>::execute_binary(
        //alg_kind_t binary_alg, const Xbyak::Xmm &dst, const Xbyak::Xmm &lhs,
        alg_kind_t binary_alg, const Xbyak_loongarch::VReg &dst, const Xbyak_loongarch::VReg &lhs,
        const T &rhs) const {

    const auto execute_cmp_binary_lam
            //= [this](const Xbyak::Xmm &dst, const Xbyak::Xmm &lhs, const T &rhs,
            = [this](const Xbyak_loongarch::VReg &dst) {
                  this->execute_cmp_binary(dst);
              };
    //helper_binary_t<avx, Xbyak::Xmm>::execute_binary<T>(
    helper_binary_t<lasx, Xbyak_loongarch::VReg>::execute_binary<T>(
            host_, execute_cmp_binary_lam, binary_alg, dst, lhs, rhs);
}
...
template class jit_uni_binary_injector_t<lasx, Xbyak_loongarch::XVReg>;
template class jit_uni_binary_injector_t<lasx, Xbyak_loongarch::VReg>;
...
template class jit_uni_binary_injector_t<lsx, Xbyak_loongarch::VReg>;
