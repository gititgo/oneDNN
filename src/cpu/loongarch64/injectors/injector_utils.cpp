...
static std::size_t get_vmm_size_bytes(const Xbyak_loongarch::VReg &vmm) {
    static constexpr int byte_size_bits = 8;
    return vmm.getBit() / byte_size_bits;
}

static std::size_t calc_vmm_to_preserve_size_bytes(
        const std::initializer_list<Xbyak_loongarch::VReg> &vmm_to_preserve) {

    return std::accumulate(vmm_to_preserve.begin(), vmm_to_preserve.end(),
            std::size_t(0u), [](std::size_t accum, const Xbyak_loongarch::VReg &vmm) {
...
register_preserve_guard_t::register_preserve_guard_t(jit_generator *host,
    ...
    if (!vmm_stack_.empty()) {
        ...
        for (const auto &vmm : vmm_to_preserve) {
            ...
            if (vmm.isVReg())
                host_->uni_xvst(Xbyak_loongarch::VReg(idx), host_->sp, stack_offset);
            else if (vmm.isXVReg())
                host_->uni_xvst(Xbyak_loongarch::XVReg(idx), host_->sp, stack_offset);
            else
                assert("unreachable");
        }
    }
...
register_preserve_guard_t::~register_preserve_guard_t() {
    ...
    while (!vmm_stack_.empty()) {
        ...
        const Xbyak_loongarch::VReg &vmm = vmm_stack_.top();
        const auto idx = vmm.getIdx();
        //if (vmm.isXMM())
        //    host_->uni_vmovups(
        ...
        if (vmm.isVReg())
            host_->uni_xvld(Xbyak_loongarch::VReg(idx), host_->sp, tmp_stack_offset);
        else if (vmm.isXVReg())
            host_->uni_xvld(Xbyak_loongarch::XVReg(idx), host_->sp, tmp_stack_offset);
        else
            assert("unreachable");
