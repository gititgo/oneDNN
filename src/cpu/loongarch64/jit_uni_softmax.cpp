        ...
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                ...
                if (tail)
                    ...
                    xvfmax_s(vmax | tail_opmask, vmax, vreg_tmp_src);
                else
                    //uni_vmaxps(vmax, vmax, vreg_tmp_src);
                    xvfmax_s(vmax, vmax, vreg_tmp_src);
            }
        });
...
struct jit_softmax_t<lasx> : public jit_softmax_base_t<lasx> {
    ...
    void movups_tail(const Xbyak_loongarch::XVReg &x, const int tail,
            const Xbyak_loongarch::XReg &addr){
    ...
    void movups_tail(const Xbyak_loongarch::XReg &addr, const int tail,
            const Xbyak_loongarch::XVReg &x){
        for (int i = 0; i < tail; i++){
            ...
            for (int i = 0; i < unroll; i++) {
                ...
                if (!tail) {
                    ...
                    if (axis_is_blocked_) {
                        ...
                        //uni_vblendvps(vzeropad, vzeropad, vreg_tmp_src, tail_vmask);
        ...
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                ...
                if (!tail) {
                    ...
                    for (size_t j = 0; j < axis_simd_tail_; j++tail_vmask) {
