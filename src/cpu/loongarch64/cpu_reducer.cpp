...
struct reducer_2d_driver_f_s_32_t : public reducer_2d_driver_t<data_type> {
    ...
    using Vmm = Xbyak_loongarch::XVReg;
    //const AddressFrame &vmmword = (isa == avx2) ? this->yword : this->zword;
    //void uni_vadd(const Xmm &x1, const Xmm &x2, const Operand &op) {
    void uni_vadd(const Vmm &x1, const Vmm &x2, const Vmm &op) {
        if (data_type == data_type::f32)
            //this->vaddps(x1, x2, op);
            this->uni_fadd_s(x1, x2, op);
        else
            this->xvadd_w(x1, x2, op);
    }
    //void uni_add(const Xmm &x1, const Operand &op) {
    void uni_add(const VReg &x1, const VReg &op) {
        if (data_type == data_type::f32) {
            //this->addss(x1, op);
            this->vfadd_s(VReg(28), x1, op);
            this->vextrins_w(x1, VReg(28), 0);
        } else {
            //this->paddd(x1, op);
            this->vadd_w(VReg(28), x1, op);
            this->vextrins_w(x1, VReg(28), 0);
        }
    }
    ...
    void load_dst(int nloads, int load_len) {
        for (int i = 0; i < nloads; ++i) {
            if (load_len == typesize) {
                ...
                this->uni_ld_w(reg_tmp, reg_dst, i * load_len);
                this->vinsgr2vr_w(VReg(i), reg_tmp, 0);
            }
            else if (load_len == vlen) {
                //this->vmovups(Vmm(i), this->ptr[reg_dst + i * load_len]);
                this->uni_xvld(Vmm(i), reg_dst, i * load_len);
            }
            else
                assert(!"unsupported");
        }
    ...
    void store_dst(int nloads, int load_len) {
        for (int i = 0; i < nloads; ++i) {
            if (load_len == typesize) 
                ...
                this->uni_xvstelm_w(VReg(i), reg_dst, i * load_len, 0);
            else if (load_len == vlen)
                //this->vmovups(this->ptr[reg_dst + i * load_len], Vmm(i));
                this->uni_xvst(Vmm(i), reg_dst, i * load_len);
            else
                assert(!"unsupported");
        }
    }
    ...
    void accumulate(int nloads, int load_len, size_t base_off) {
        for (int i = 0; i < nloads; ++i) {
            ...
            if (load_len == typesize) {
                ...
                this->uni_xvld(VReg(28), reg_src, off);
                this->uni_add(VReg(i), VReg(28));
            }
            else if (load_len == vlen) {
                //this->uni_vadd(Vmm(i), Vmm(i), vmmword[reg_src + off]);
                this->uni_xvld(Vmm(28), reg_src, off);
                this->uni_vadd(Vmm(i), Vmm(i), Vmm(28));
            }
            else
                assert(!"unsupported");
        }
    ...
    void loop_x() {
        ...
        for (int id = 0; id < nbranches; ++id) {
            ...
            if (nloads[id] > 1) {
                ...
                this->add_imm(reg_src, reg_src, this->src_ld_ * typesize, reg_tmp);

                //this->dec(reg_src_id);
                this->addi_d(reg_src_id, reg_src_id, -1);
                //this->jnz(loop_srcs, this->T_NEAR);
                this->bnez(reg_src_id, loop_srcs);

                size_t base_off
                        = (size_t)this->n_src_ * this->src_ld_ * typesize;
                //this->safe_sub(reg_src, base_off, reg_long_offt);
                this->add_imm(reg_src, reg_src, -1 * base_off, reg_tmp);
            } else {
            ...
            this->add_imm(reg_src, reg_src, nloads[id] * load_len[id], reg_tmp);
            //this->add(reg_dst, nloads[id] * load_len[id]);
            this->add_imm(reg_dst, reg_dst, nloads[id] * load_len[id], reg_tmp);

            //this->sub(reg_x, nloads[id] * load_len[id]);
            this->add_imm(reg_x, reg_x, -1 * nloads[id] * load_len[id], reg_tmp);
        ...
        this->sub_d(reg_src, reg_src, reg_nx);
        //this->sub(reg_dst, reg_nx);
        this->sub_d(reg_dst, reg_dst, reg_nx);
    }
    ...
    void generate() override {
        ...
        this->add_imm(reg_dst, reg_dst, this->dst_step_ * typesize, reg_tmp);
        //this->add(reg_src, this->src_step_ * typesize);
        this->add_imm(reg_src, reg_src, this->src_step_ * typesize, reg_tmp);

        //this->dec(reg_ny);
        this->addi_d(reg_ny, reg_ny, -1);
        //this->jnz(ny_loop, this->T_NEAR);
        this->bnez(reg_ny, ny_loop);
...
inline reducer_2d_driver_t<data_type> *create_reduce_2d_drv(int n_src,
    ...
    if (mayiuse(lasx))
        return new reducer_2d_driver_f_s_32_t<data_type, lasx>(
                n_src, src_ld, src_step, dst_step, nullify_dst);
